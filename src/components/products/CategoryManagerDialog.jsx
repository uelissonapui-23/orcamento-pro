...
import { createProductCategory, updateProductCategory } from "../../services/productService";

export default function CategoryManagerDialog({ open, workspaceId, categories, onClose, onChanged }) {
  ...
  const create = async (event) => {
    ...
    try {
      await createProductCategory(workspaceId, newName);
      setNewName("");
      await onChanged();
    } catch (err) {
  ...
  const saveEdit = async (category) => {
    ...
    try {
      await updateProductCategory(category.id, { name: editingName.trim() });
      setEditingId("");
      await onChanged();
    } catch (err) {
  ...
  const toggle = async (category) => {
    ...
    try {
      await updateProductCategory(category.id, { active: !category.active });
      await onChanged();
    } catch (err) {
    ...
    <div className="dialog-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <section className="category-dialog" role="dialog" aria-modal="true">
        <header className="dialog-header">
          <div>
            <h2>Categorias</h2>
            <p>Organize produtos e serviços sem criar módulos separados.</p>
          </div>
