...
export function defaultAutomationSettings(workspaceId) {
  return {
    workspace_id: workspaceId,
    quote_followup_days: 3,
    quote_expiry_warning_days: 2,
    delivery_warning_days: 2,
    show_safe_suggestions: true,
    default_whatsapp_message:
      "Olá, {cliente}! Estou entrando em contato sobre o orçamento #{numero}.",
  };
}

export function emptyAutomationSuggestions() {
  return {
    followups: [],
    expiring: [],
    deliveries: [],
...
export async function getAutomationSettings(workspaceId) {
  ...
  return data || defaultAutomationSettings(workspaceId);
}
...
export async function toggleTemplateFavorite(workspaceId, template) {
  return saveQuoteTemplate(workspaceId, {
    ...template,
...
export async function getAutomationSuggestions(workspaceId, settings) {
  ...
  const followupCutoff = new Date(
    now.getTime() - Number(settings.quote_followup_days || 3) * 86400000,
  ).toISOString();

  const expiryLimit = new Date(
    now.getTime() + Number(settings.quote_expiry_warning_days || 2) * 86400000,
  )
    .toISOString()
    .slice(0, 10);

  const deliveryLimit = new Date(
    now.getTime() + Number(settings.delivery_warning_days || 2) * 86400000,
  )
    .toISOString()
    .slice(0, 10);
  ...
  const [follow, expiring, deliveries, recent] = await Promise.all([
    db
      .schema("orcamento_app")
      .from("quotes")
      .select("id,quote_number,client_snapshot_json,updated_at")
      .eq("workspace_id", workspaceId)
      .eq("status", "awaiting_response")
      .lte("updated_at", followupCutoff)
      .order("updated_at")
      .limit(8),
    ...
    db
      .schema("orcamento_app")
      .from("quotes")
      .select("id,quote_number,client_snapshot_json,valid_until")
      .eq("workspace_id", workspaceId)
      .eq("status", "awaiting_response")
      .gte("valid_until", today)
      .lte("valid_until", expiryLimit)
      .order("valid_until")
      .limit(8),
    ...
    db
      .schema("orcamento_app")
      .from("work_orders")
      .select("id,quote_id,quote_number,client_snapshot_json,due_date,status")
      .eq("workspace_id", workspaceId)
      .in("status", ["pending", "in_progress", "ready"])
      .lte("due_date", deliveryLimit)
      .order("due_date")
      .limit(8),
    ...
    db
      .schema("orcamento_app")
      .from("quotes")
      .select("id,quote_number,client_snapshot_json,status,updated_at")
      .eq("workspace_id", workspaceId)
      .order("updated_at", { ascending: false })
      .limit(6),
  ]);

  for (const result of [follow, expiring, deliveries, recent]) {
